...
Installs host packages, a tools virtualenv (esptool and PlatformIO), ESP-IDF, udev rules, and `dialout` membership, then writes `activate.sh`.

## Overview

`setup` and `resume` run the same code. Both start by deleting leftover `*.partial` staging directories and incomplete `esp-idf` / `venv` trees, then continue. A complete ESP-IDF clone is kept: git fetch/checkout, submodule update, and `install.sh` run again.

Linux and Python 3.10+ are required. Default prefix is `~/.esp32-dev`.

ESP-IDF version defaults to the latest GitHub release (`latest` or `stable` are aliases). Pin a tag with `--idf-version v6.1`. esptool and PlatformIO are `pip install --upgrade` with no upper bound.
...
Both print one row per check: git, cmake, ninja, python3, venv, esptool, platformio, esp-idf, udev, dialout. `status` always exits 0 (unless the command itself fails). `verify` exits 1 if any of git, python3, esptool, platformio, or esp-idf is missing. cmake, ninja, udev, and dialout are reported but not required for a passing `verify`.
...
Missing `git`/`cmake`/`ninja` after package install (when IDF is not skipped) also fails.
...
**Killed mid-clone**: re-run `setup` or `resume`. Incomplete trees are removed; a usable clone is reused.
